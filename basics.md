...
| Prompt Tip | Example |
|---|---|
| **Start Simple, Iterate** - Begin with a broad request, then refine it. | "Summarize text" → "Summarize text in 3 bullet points." |
| **Use Clear Action Verbs** - Tell the model precisely what to do. | "Write a product description." |
| **Place Instructions First** - Put your main command at the beginning of the prompt. | "### Instruction ### Translate to Spanish: Text: Hello." |
| **Use Separators** - Clearly delineate different parts of your prompt. | "### Task ### Classify sentiment ### Text ### I love this." |
| **Be Specific & Detailed** - Provide ample context and explicit requirements. | "Write a formal email to a client apologizing for a 2-day shipping delay." |
| **Include Example Outputs** - Show the model the desired format and style. | "Extract names. Format: Name: John, Mary from text: John and Mary." |
| **Keep Details Relevant** - Avoid unnecessary or distracting information. | (Avoid) "Write a story about dogs, make it interesting and engaging." |
| **Be Direct, Not Clever** - Prioritize clarity and straightforwardness. | "Explain quantum physics to a 12-year-old in 2 sentences" (better than "Explain quantum physics simply"). |
| **State What To Do** - Use positive instructions instead of negative ones. | "Recommend movies from trending list" vs. "Don't ask for personal preferences." |
| **Experiment Extensively** - Try different phrasing and approaches. | Try "List pros and cons" then "Create a comparison table." |
| **Break Down Complex Tasks** - Divide a big problem into smaller, manageable steps. | "Analyze market data" → "1. Identify trends 2. Calculate averages." |
| **Balance Detail with Length** - Include necessary context without being overly verbose. | Provide key context but avoid lengthy background information. |
| **Use Proper Formatting** - Apply structured inputs and outputs like newlines. | "Task: Classify\nInput: This movie was amazing\nOutput:" |
| **Structure Prompts Clearly** - Define the model's role, the task, and the audience. | "Role: Teacher\nTask: Explain photosynthesis\nAudience: 5th graders." |
| **Define Output Audience/Persona** - Tailor the output to a specific type of recipient. | "Write a product review for a tech-savvy gamer." |
| **Specify Output Format** - Explicitly request formats like JSON, tables, or lists. | "Extract entities into a JSON array." |
| **Manage Ambiguity** - Instruct the model on how to handle unclear information. | "If any part is unclear, state the ambiguity rather than making an assumption." |
| **Use Chain-of-Thought** - Guide the model to show its step-by-step reasoning. | "First, identify the premise. Second, analyze its validity. Finally, provide your assessment." |
| **Assign Model Persona** - Instruct the model to adopt a specific character or tone. | "You are a seasoned financial analyst. Explain the market trends." |
